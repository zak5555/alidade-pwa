...
async function main() {
    ...
    const targetTier = parseArg('--tier', 'ultimate').toLowerCase();
    const licenseKey = parseArg('--license-key', '');
    ...
    const users = await supabaseRequest(
        baseUrl,
        serviceRoleKey,
        '/rest/v1/users?select=id,email,license_tier,license_key,created_at&order=created_at.desc&limit=1',
        'GET'
    );

    if (!Array.isArray(users) || users.length === 0) {
        console.log('[PROMOTE_LATEST_USER] No users found.');
        return;
    }

    const latestUser = users[0];
    const updatePayload = { license_tier: targetTier };
    if (licenseKey) {
    ...
    const updatedRows = await supabaseRequest(
        baseUrl,
        serviceRoleKey,
        `/rest/v1/users?id=eq.${encodeURIComponent(String(latestUser.id || ''))}`,
        'PATCH',
        updatePayload
    );
    ...
    console.log(JSON.stringify({
        ...
        targetUser: {
            ...
            beforeTier: latestUser.license_tier || null
        },
        updated: Array.isArray(updatedRows) ? updatedRows[0] || null : updatedRows
    }, null, 2));
}

main().catch((error) => {
    console.error('[PROMOTE_LATEST_USER] FAIL');
